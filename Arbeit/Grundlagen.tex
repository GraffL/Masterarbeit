...
\begin{defn}
	Eine Folge von Strategieprofilen $x^0, x^1, x^2, \dots$ ist 
	\begin{itemize}
		...
		\item ein \emph{Verbesserungspfad}, wenn sie ein Pfad ist und sich der abweichende Spieler jeweils echt verbessert. Das heißt es gilt an jeder Stelle $n$: $c_{i(k)}(x^{k+1}) \cordle c_{i(k)}(x^k)$.
		\item ein \emph{abgeschlossener Verbesserungspfad}, wenn sie ein endlicher Verbesserungspfad ist, der nicht mehr nach hinten verlängert werden kann, d.h. es keine Strategie $\hat{x}_i$ gibt, mit $c_{i}(x^n_{-i}, \hat{x}_i) \cordle c_{i}(x^n)$.\todo{Schöner formulieren}
	\end{itemize}

	\todo[inline]{Zykel, bester Antwort Pfad}
	
	Für einen endlichen Pfad $\gamma = (x^0, \dots, x^n)$ ist die \emph{Gesamtänderung} entlang des Pfades definiert als die Summe aller Änderungen für die jeweils abweichenden Spieler:
