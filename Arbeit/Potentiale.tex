...
\begin{defn}
	...
	\begin{itemize}
		...
		\item \defIndexSub{lokales Nash-Potential}{Potential}{lokales Nash-}, wenn jedes \glqq lokale\grqq{} Minimum von $P$ ein Nash-Gleichgewicht in $\Gamma$ ist und umgekehrt, d.h. für alle $x \in X$ gilt:
			\[\forall i \in I, \hat{x}_i \in X_i: P(x) \leq P(x \mid \hat{x}_i) \iff \forall i \in I, \hat{x}_i \in X_i: c_i(x) \leq c_i(x \mid \hat{x}_i) \]
		\item \defIndexSub{Beste-Antwort-Potential}{Potential}{Beste-Antwort-}, wenn für jeden Spieler $i$ und alle Strategieprofile $x \in X$ gilt:
			\[\arg\min_{\hat{x}_i \in X_i}c_i(x \mid \hat{x}_i) = \arg \min_{\hat{x}_i \in X_i} P(x \mid \hat{x}_i)\]
		\item \defIndexSub{verallgemeinertes ordinales Potential}{Potential}{verallgemeinertes ordinales}, wenn für jeden Spieler $i$ und alle Strategieprofile $x \in X$ sowie $\hat{x}_i \in X_i$ gilt:
			\[c_i(x) > c_i(x \mid \hat{x}_i) \implies P(x) > P(x \mid \hat{x}_i)\]
		\item \defIndexSub{ordinales Potential}{Potential}{ordinales}, wenn für jeden Spieler $i$ und alle Strategieprofile $x \in X$ sowie $\hat{x}_i \in X_i$ gilt:
			\[c_i(x) > c_i(x \mid \hat{x}_i) \iff P(x) > P(x \mid \hat{x}_i)\]
		\item \defIndexSub{skaliertes Potential}{Potential}{skaliertes}, wenn es streng monotone Funktionen $f_i: \IR \to \IR$ gibt, sodass für jeden Spieler $i$ und alle Strategieprofile $x \in X$ sowie $\hat{x}_i \in X_i$ gilt:
			\[c_i(x) - c_i(x \mid \hat{x}_i) = f_i(P(x)) - f_i(P(x \mid \hat{x}_i))\]
		\item \defIndexSub{gewichtetes Potential}{Potential}{gewichtetes}, wenn es einen Gewichtsvektor $(w_i)_{i\in I} \in \IR_{>0}^I$ gibt, sodass für jeden Spieler $i$ und alle Strategieprofile $x \in X$ sowie $\hat{x}_i \in X_i$ gilt:
			\[c_i(x) - c_i(x \mid \hat{x}_i) = w_i\cdot(P(x) - P(x \mid \hat{x}_i))\]
		\item \defIndexSub{exaktes Potential}{Potential}{exaktes}, wenn für jeden Spieler $i$ und alle Strategieprofile $x \in X$ sowie $\hat{x}_i \in X_i$ gilt:
...
\begin{satz}\label{satz:CharExPot}\index{Potential!exaktes}
...
\begin{satz}\label{satz:CharExPotAlt}\index{Potential!exaktes}
...
\begin{satz}\label{satz:CharGewPot}\index{Potential!gewichtetes}
...
\begin{satz}\label{satz:CharSkalPot}\index{Potential!skaliertes}
...
\begin{satz}\label{satz:CharOrdPot}\index{Potential!ordinales}
...
\begin{satz}\label{satz:CharVerallOrdPot}\index{Potential!verallgemeinertes ordinales}
...
\begin{satz}\label{satz:CharExBAPot}\index{Potential!Beste-Antwort-}
...
\begin{proof}
	Angenommen $\Gamma$ enthielte einen schwachen Beste-Antwort-Verbesserungszykel $\gamma = (x^0, \dots, x^n)$, so wäre für alle $0 < k \leq n$
...
\begin{proof}[Beweis von \Cref*{satz:CharExBAPot}]
	...
	\begin{description}
		...
		\item[\glqq$\bm{\supseteq}$\grqq:] Sei $\hat{x}_i \in \arg \min_{\hat{x}_i \in X_i}P(x \mid \hat{x}_i)$. Angenommen es wäre $\hat{x}_i \notin \arg \min_{\hat{x}_i \in X_i}c_i(x \mid \hat{x}_i)$, d.h. es gäbe eine beste Antwort $x'_i \in X_i$ auf $x$ mit $c_i(x \mid x'_i) < c_i(x \mid \hat{x}_i)$. Dann wäre $((x \mid \hat{x}_i), (x \mid x'_i))$ ein Beste-Antwort-Verbesserungspfad und daher $[(x \mid x'_i)] \BAord [(x \mid \hat{x}_i)]$. Somit wäre auch $P(x \mid x'_i) = f([x \mid x'_i]) < f([x \mid \hat{x}_i]) = P(x \mid \hat{x}_i)$, im Widerspruch zu $\hat{x}_i \in \arg \min_{\hat{x}_i \in X_i}P(x \mid \hat{x}_i)$. Also gibt es kein solches $x'_i$ und es gilt $\hat{x}_i \in \arg \min_{\hat{x}_i \in X_i}c_i(x \mid \hat{x}_i)$. \qedhere
	\end{description}
\end{proof}
