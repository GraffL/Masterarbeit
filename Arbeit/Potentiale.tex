...
\begin{proof}
	Wir folgen dem Beweis aus \cite[Anhang A]{MonShap}. Sei zunächst $\gamma \coloneqq (x^0, x^1, x^2, x^3, x^4)$ ein beliebiger 4-Zykel in einem Spiel mit Potential $P$. Dann gilt für die Gesamtänderung:
		\[\delta(\gamma) = \sum_{i=0}^3 c_{i(k)}(x^{k+1}) - c_{i(k)}(x^k) = \sum_{k=0}^{3} P(x^{k+1}) - P(x^k) = P(x^4) - P(x^0) = 0^{}\]
		
	Ist umgekehrt $\Gamma$ ein Spiel, in dem für alle 4-Zykel $\gamma$ gilt $\PfadAend(\gamma) = 0$, und $\hat{x}$ ein beliebiges, aber festes Strategieprofil in $\Gamma$\todo{Für Spiele mit unendlich vielen Spielern sollte hier ein Strategieprofil pro Zusammenhangskomponente gewählt werden!}. Dann definiere wie folgt eine Funktion $P$ auf dem Strategieraum:
		\[P: X \to \IR: x \mapsto \PfadAend(\gamma), \gamma \text{ beliebiger Pfad von } \hat{x} nach x \]
