...
\begin{satz}\label{satz:CharOrdPot}
	Ein Spiel besitzt genau dann ein ordinales Potential, wenn es keine schwachen Verbesserungszykel enthält und $([X], \nVerschlRel)$ reell geordnet ist.
\end{satz}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item $([X], \nVerschlRel)$ ist reell geordnet. Definiere dazu die Abbildung $f: X/{\approx} \to \IR: [x] \mapsto P(x)$. Diese ist wohldefiniert, denn ist $y \in [x]$, so gibt es also nicht-Verschlechterungspfade von $x$ nach $y$ und umgekehrt. Zusammen bilden dieses einen nicht-Verschlechterungszykel und da es keine schwachen Verbesserungszykel in $\Gamma$ gibt, muss in die diesem Zykel (und damit bereits in den beiden Pfaden) in jedem Schritt Gleichheit gelten. Insbesondere folgt damit $P(x) = P(y)$.
		
		Ferner ist diese Abbildung streng monoton, denn gilt $[x] \nVerschlRel [y]$, so gibt es einen nicht-Verschlechterungspfad $\gamma$ von $y$ nach $x$, aber keinen in die umgekehrte Richtung. Insbesondere ist also $\overset{\leftarrow}{\gamma}$ kein nicht-Verschlechterungspfad, d.h. $\gamma$ ein schwacher Verbesserungspfad von $y$ nach $x$. Damit folgt analog zum ersten Punkt: $P(x) < P(y)$
	\end{enumerate}

	Ist umgekehrt $\Gamma$ ein Spiel ohne schwache Verbesserungszykel und $([X], \nVerschlRel)$ reell-geordnet mit Abbildung $f: [X] \to \IR$, so definiere $P: X \to \IR: x \mapsto f([x])$. Dies ist ein ordinales Potential, denn es gilt:
	\begin{enumerate}
...
In dieser Form ist dies eine wenig hilfreiche Charakterisierung, da sie Frage nach der Existenz eines verallgemeinerten ordinalen Potential durch die eine noch schwerer Frage ersetzt.\todo{Besser formulieren!}{} Allerdings erhält man daraus mit Hilfe der folgenden Proposition einfachere Charakterisierungen für gewisse Teilklassen von Spielen:
...
\begin{proof}
	Nach \cite[Lemma 2.2]{CharExOrdPot} ist jede abzählbare Menge mit einer strikten Partialordnung bereits reell geordnet. Also macht $\VerbRel$ einen abzählbaren Strategieraum genau dann zu einer reell geordneten Menge, wenn $\VerbRel$ eine strikte Partialordnung ist. Und dies ist nach \Cref{prop:VerRelPartOrdVerbz} genau dann der Fall, wenn das Spiel keine Verbesserungszykel enthält.
\end{proof}

\begin{kor}
	Ein endliches Spiel besitzt genau dann ein verallgemeinertes Potential, wenn es die FIP besitzt.
\end{kor}

\begin{proof}
	In einem Spiel mit endlichem Strategieraum sind unendliche Verbesserungspfade genau die Verbesserungskreise. Damit folgt die Aussage direkt aus dem vorherigen Korollar.
\end{proof}

Diese Charakterisierung der Existenz verallgemeinerter ordinaler Potentiale wurde (auf anderem Wege) erstmals von \citeauthor{MonShap} in \cite[Lemma 2.5]{MonShap} gezeigt.

\begin{kor}
	Ein Spiel mit abzählbarer Spielermenge besitzt genau dann ein verallgemeinertes ordinales Potential, wenn es keine Verbesserungskreise enthält.
\end{kor}

\begin{proof}
	Nach \cite[Lemma 2.3]{CharExOrdPot} genügt es für eine Menge mit strikter Partialordnung bereits, wenn sie eine abzählbare, bezüglich dieser Ordnung Teilmenge besitzt, um reell geordnet zu sein. Dabei heißt eine Teilmenge $Y$ einer Menge $X$ mit strikter Partialordnung $<$ \emph{dicht bezüglich der Ordnung $<$}, wenn gilt:
		\[\forall x,z \in X\setminus Y: x < z \implies \exists y \in Y: x < y < z \]
	Zusammen mit \Cref{prop:VerRelPartOrdVerbz} genügt es für den Beweis des Korollars also eine $\VerbRel$-ordnungsdichte Teilmenge des Strategieraumes zu finden.
	
	Dazu definieren wir für jeden Spieler $i \in I$, eine (abzählbare) Teilmenge
		\[Y_i := \left\{x^{i,q} \,\middle|\, q \in \IQ, x^{i,q} \in X: \abs{c_i(x^{i,q})-q} = \min_{x \in X}\abs{c_i(x) - q}\right\}\]
	Das heißt zu jeder rationalen Zahl $q$ enthält $Y_i$ ein Strategieprofil, dessen Kosten für Spieler $i$ möglichst nahe an $q$ liegen.
	
	Es ist nun $Y := \bigcup_{i \in I} Y_i \subseteq X$ eine abzählbare Menge (da $I$ abzählbar ist). Außerdem liegt $Y$ bezüglich der Ordnung $\VerbRel$ dicht in $X$, denn es gilt:
	
	Seien $x$ und $y$ zwei Strategieprofile aus $X\setminus Y$ mit $x < z$, dann gibt es also einen Verbesserungspfad $\gamma = (x^0, x^1, \dots, x^n)$ mit $x^0 = z$ und $x^n = x$. Insbesondere gilt für den als erstes abweichenden Spieler $i(0)$, dass $c_{i(0)}(x^1) \cordle c_{i(0)}(x^0)$. Da $\IQ$ dicht in $\IR$ liegt, gibt es nun eine rationale Zahl $q$ mit $c_{i(0)}(x^1) \cordle q \cordle c_{i(0)}(x^0)$. Nach Definition von $Y_i$ gilt nun für $x^{i,q} \in Y_i \subseteq Y$:
		\[c_{i(0)}(x^1) \cordleq c_{i(0)}(x^{i,q}) \cordleq c_{i(0)}(x^0)\]
	
	\todo[inline]{Vermutlich funktioniert das so nicht...}
\end{proof}

\todo[inline]{alternativen (konstruktiven!) Beweis \cite{CongGamesPlayerSpecPayoff} erwähnen?}
