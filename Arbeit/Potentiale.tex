...
\begin{bsp}
	...
	\begin{itemize}
		\item $\Gamma_4$ besitzt ein verallgemeinert ordinales und ein Beste-Antwort-Potential ($P_4$), aber kein ordinales Potential:
			\begin{center}
				$\Gamma_4:$ \quad
...
\begin{proof}[Beweis von \Cref*{satz:CharOrdPot}]
	Sei zunächst $P: X \to \IR$ ein ordinales Potential eines Spiels $\Gamma$. Nach \Cref{prop:OrdPotKeineschwVBZ} gibt es in $X$ dann keine schwachen Verbesserungszykel, woraus mit \Cref{prop:SVordStrPO} folgt, dass $\SVord$ eine strikte Partialordnung ist. Wir müssen also noch zeigen, dass $(\XmodNV, \SVord)$ reell geordnet ist. 
	...
	Ist umgekehrt $(\XmodNV, \SVord)$ reell-geordnet mit Abbildung $f: \XmodNV \to \IR$, so definiere $P: X \to \IR: x \mapsto f([x])$. Dies ist ein ordinales Potential, denn es gilt:
	\begin{enumerate}
		\item Gilt $c_i(x) > c_i(x \mid \hat{x}_i)$, so ist $(x, (x \mid \hat{x}_i))$ ein schwacher Verbesserungspfad, also $[x] \succ [(x \mid \hat{x}_i)]$. Daraus wiederum folgt $P(x) = f([x]) > f([(x \mid \hat{x}_i)]) = P(x \mid \hat{x}_i)$.
...
\begin{proof}
	...
	\[Y_x^n \coloneqq \left\{y \in X \setMid \text{es existiert ein Pfad der Länge $n$ von $x$ nach $y$}\right\}, \]
	...
	\begin{description}
		...
		\[Y_x^1 = \left\{y \in X \setMid \exists i \in I \exists y_i\in Y_i : y = (x \mid y_i)\right\} = \left\{(x \mid y_i) \setMid i \in I, y_i \in Y_i \right\} = \bigcup_{i \in I} \left\{(x \mid y_i) \setMid y_i \in Y_i \right\}\]
	...
	\end{description}
	Folglich ist auch $Y_x = \bigcup_{n\in\INs} Y_x^n$ abzählbar und besitzt daher nach \Cref{kor:CharExOrdPotabzX} ein ordinales Potential.
\end{proof}
...
\begin{proof}
	...
	Ferner ist $\VerbRel$ genau dann irreflexiv, wenn für alle $x \in X$ gilt, dass $x \not\VerbRel x$, es also keinen Verbesserungspfad von $x$ nach $x$ gibt. Und letzteres entspricht gerade einem (bei $x$ beginnenden) Verbesserungskreis.
\end{proof}
...
Erneut können wir daraus für \glqq kleine\grqq{} Spiele einfachere Charakterisierungen erhalten (die Beweise sind ganz analog zu denen von \Cref{kor:CharExOrdPotabzX} und \Cref{kor:CharExOrdPotabzIundXi}):
...
Beschränkt man sich hingegen auf die Klasse der endlichen Spielen, so erhält man die (auf anderem Wege) erstmals von \citeauthor{MonShap} in \cite[Lemma 2.5]{MonShap} gezeigte Charakterisierung der Existenz von verallgemeinerten ordinalen Potentialen:
...
\begin{proof}
	Angenommen $\Gamma$ enthielte einen Verbesserungszykel $\gamma = (x^0, \dots, x^n)$ wäre ein schwacher Beste-Antwort-Verbesserungszykel in $\Gamma$, so gilt für alle $0 < k \leq n$:
...
\begin{proof}[Beweis von \Cref*{satz:CharExBAPot}]
	Sei zunächst $P: X \to \IR$ ein Beste-Antwort-Potential eines Spiels $\Gamma$. Dann gibt es in $\Gamma$ \Cref{prop:BAPotKeineschwBAVBZ} keine schwachen Beste-Antwort-Verbesserungszykel und daher ist $\BAord$ nach \Cref{prop:BARelPartOrdVerbz} eine strikte Partialordnung auf $\XmodBA$.
	
	Ferner ist $(\XmodBA, \BAord)$ reell geordnet. Definiere dazu die Abbildung $f: \XmodBA \to \IR: [x] \mapsto P(x)$. Diese ist wohldefiniert, denn ist $y \in [x]$, so gibt es also Beste-Antwort-Pfade von $x$ nach $y$ und umgekehrt. Zusammen bilden dieses einen Beste-Antwort-Zykel und da es keine schwachen Beste-Antwort-Verbesserungszykel in $\Gamma$ gibt, muss in die diesem Zykel (und damit bereits in den beiden Pfaden) in jedem Schritt Gleichheit gelten. Insbesondere folgt damit $P(x) = P(y)$.
	...
	Ist umgekehrt $\Gamma$ ein Spiel, für das $(\XmodBA, \BAord)$ reell geordnet ist mit Abbildung $f: \XmodBA \to \IR$, so definiere $P: X \to \IR: x \mapsto f([x])$. Dies ist ein beste Antwort-Potential, denn es gilt $\arg \min_{x'_i \in X_i}c_i(x \mid x'_i) = \arg \min_{x'_i \in X_i} P(x \mid x'_i)$:
	\begin{description}
...
\begin{sidewaystable}\centering\renewcommand*{\arraystretch}{1.2}\Crefname{beob}{Beob.}{Beob.}\Crefname{kor}{Kor.}{Kor.}
	\begin{tabularx}{\textwidth}{ccXr}
		...
		-f. $I$ und $X_i$ abzählbar:	& $\iff$	& $\Gamma$ enthält keine schwachen Verbesserungszykel 
