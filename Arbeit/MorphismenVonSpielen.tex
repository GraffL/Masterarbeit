...
\begin{defn}\label{def:PotentialMorphismen}
	...
	\begin{itemize}
		...
		\item \emph{biordinal}, wenn für alle Strategieprofile $x \in X$, Spieler $j \in J$ und Strategien $\hat{x}_{\sigma(j)}$ gilt:\todo{In die nachfolgenden Sätze einpflegen? Oder ausgelagert als Bemerkung definieren, da es nur ein einziges Mal verwendet wird?}
...
\begin{bem}
	Ist ein Morphismus exakt/gewichtet/skalierend/biordinal und ein Isomorphismus so ist er automatisch ein exakter/gewichteter/skalierender/biordinaler Isomorphismus (analog etwa zu Gruppenhomomorphismen). Bei ordinalen oder Beste-Antwort-Morphismen muss zusätzlich geprüft werden, ob auch der inverse Morphismus ordinal bzw. Beste-Antwort ist (analog beispielsweise zu stetigen Abbildungen).\todo{Stimmt das so?}
\end{bem}
