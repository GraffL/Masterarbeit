...
Wie wir im vorherigen Kapitel gesehen haben, definieren Potentiale ein Koordinationsspiel, welches in einer \glqq gewissen Beziehung\grqq{} zum Ausgangsspiel steht (etwa die gleichen Nash-Gleich\-ge\-wich\-te besitzt). Genau diese Beziehung wollen wir nun mit Hilfe eines Morphismus bzw. eines Isomorphismus zwischen den beiden Spielen präziser beschreiben. Die Eigenschaften, die Ausgangs- und Koordinationsspiel gemeinsam haben, sind dann genau die, die von den jeweiligen (Iso-)Morphismen bewahrt werden. Umgekehrt ist es auch möglich, zuerst bestimmte Arten von Isomorphimen zu definieren und daraus dann neue Potentialbegriffe zu gewinnen, wie das beispielsweise in \cite[Definitionen 5/6]{BestRespEq} getan wird.

Darüber hinaus helfen Morphismen auch dabei, andere Beziehungen zwischen verschiedenen Spielen zu verstehen. Bereits \citeauthor{Nash} verwendet in \cite[S. 288f]{Nash} den Begriff eines Automorphismus (bzw. einer Symmetrie) von Spielen. Auch die von \citeauthor{MonShap} in \cite[Theorem 3.2]{MonShap} gefundene Äquivalenz von Auslastungsspielen und Spielen mit exaktem Potential wird durch einen passenden Isomorphiebegriff beschrieben. Und in \Cref{sec:Auslastungsspiele} werden wir später noch weitere Beispiele für derartige Verbindungen kennenlernen. Schließlich ist es aus kategorientheoretischer Sicht auch allgemein interessant, die Morphismen der Kategorie, in der man arbeitet, zu verstehen, da diese die Struktur dieser Kategorie festlegen und etwa dazu verwendet werden, abstrakte Konstruktionen durchzuführen. Diesen Ansatz werden wir hier allerdings nicht fortführen und verweisen dazu auf \cite{LapGameCat}.

\subsection{Definitionen}

\begin{defn}\label{defn:MorphismusVonSpielen}
	Zu zwei gegebenen Spielen $\Gamma = (I, X, (c_i)_{i\in I})$ und $\Delta = (J, Y, (d_j)_{j\in J})$ kann man wie folgt einen \defIndex{Morphismus} $(\sigma, \phi): \Gamma \to \Delta$ zwischen diesen beiden definieren:
	\begin{itemize}
...
Diese Art und Weise Abbildungen zwischen zwei Spielen zu definieren ergibt sich aus dem noch deutlich allgemeineren Ansatz hierzu von \citeauthor{LapGameCat} in \cite{LapGameCat}\footnote{\citeauthor{LapGameCat} lässt in seiner Definition allerdings alle beteiligten Abbildungen in die jeweils umgekehrte Richtung gehen. Wir betrachten hier also gewissermaßen die zur dort definierten duale Kategorie.}. Wir orientieren uns dabei vor allem an dem dort in Kapitel 3 vorgestellten Morphismusbegriff für topologische Spiele (in denen die Spielermenge ein topologischer Raum und der Strategieraum eine Garbe über diesem ist) bzw. dessen Spezialisierung auf Spiele mit diskreter Spielermenge aus Kapitel 4. 

Im dortigen Kontext ergibt es sich etwas natürlicher, dass die Abbildung zwischen den Spielermengen in die entgegengesetzte Richtung zu denen zwischen den Strategieräumen verläuft. Eine andere, auch im hier vorliegenden Kontext nachvollziehbare Begründung hierfür liefert aber die folgende Bemerkung:
...
\begin{bem}
	Möchte man Spieler- und Strategieabbildungen in die gleiche Richtung haben und trotzdem Morphismen zwischen Spielen mit unterschiedlich großen Spielermengen erlauben, so kann man einem alternativen Ansatz zur Definition von Morphismen von Spielen folgen, welcher von \citeauthor{Foundations} in \cite[Kapitel 1, Abschnitt 1.14]{Foundations} verwendet wird: Hierbei wird als Spielerabbildung eine mengenwertige Abbildung verwendet, die also einen einzelnen Spieler nicht zwangsläufig wieder auf einen einzelnen Spieler abgebildet, sondern gleich auf eine ganze Teilmengen der Bildspielermenge. Von diesem Morphismentyp betrachtet \citeauthor{Foundations} dann verschiedene \glqq approximativ kostenerhaltende\grqq{} Varianten und untersucht die sich dadurch ergebende Kategorie.
\end{bem}
...
\begin{bem}
	Zwei Spiele sind also genau dann äquivalent, wenn sie sich ausschließlich durch Umbenennung der Spieler und ihrer Strategien ineinander überführen lassen. In \cite[S. 133]{MonShap} wird genau das als Isomorphie von Spielen bezeichnet.
\end{bem}
...
\begin{bsp}\label{bsp:Koordinationsspiel}
	...
	Denn ist $\Delta$ ein Koordinationsspiel, so definiert man ein 1-Personenspiel $\Kappa \coloneqq (\{\ast\}, X, (c_\ast))$ mit $X = X_\ast \coloneqq Y$ und $c_\ast(y) \coloneqq d_{\hat{\jmath}}(y)$ für einen beliebigen, aber festen Spieler $\hat{\jmath} \in J$. Ferner definiert man den folgenden Morphismus $(\sigma, \phi): \Kappa \to \Delta$:
	\begin{align*}
...
\begin{defn}
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	Einen solchen Morphismus nennen wir \emph{monotonen Isomorphimus}, wenn er ein monotones Inverses hat, dessen Kostenabbildungen verknüpft mit denen von $(\sigma, \phi)$ die Identität ergeben.
\end{defn}
...
\begin{bem}
	Gibt es eine derartige Abbildung $f_j$ für einen Spieler $j \in J$, so gilt für je zwei Strategieprofile $x, \hat{x} \in X$:
		...
		\end{align}
	Ist umgekehrt diese Bedingung für einen Spieler $j \in J$ und je zwei Strategieprofile erfüllt und ist zudem $d_j(\phi(X)) \subseteq \IR$ beschränkt oder $c_{\sigma(j)}(X) \subseteq \IR$ in beide Richtungen unbeschränkt, so gibt es eine monotone Kostenabbildung $f_j$ mit $f_j \circ c_{\sigma(j)} = d_j \circ \phi$.
\end{bem}
...
\begin{proof}
	...
	\begin{center}
			\begin{tikzcd}
			X \arrow[r, "\phi"] \arrow[d, "c_i"] \arrow[rr, bend left=60, "\id"]	& Y \arrow[r, "\phi^{-1}"] \arrow[d, "d_{\sigma^{-1}(i)}"] 	& X \arrow[d, "c_i"] \\
			\IR \arrow[r, "f_{\sigma^{-1}(i)}"'] \arrow[rr, bend right=60, "\id"']	& \IR \arrow[r, "g_i"']										& \IR
		\end{tikzcd}
	\end{center}
	Es ist $P: X \to \IR: x \mapsto d_j \circ \phi(x)$ (für einen beliebigen, aber festen Spieler ${\hat{\jmath}} \in J$) ein skaliertes Potential mit Skalierungsfunktionen $g_i$, denn es gilt allgemein
		\[g_i \circ d_{\hat{\jmath}} \circ \phi \overset{\Kappa \text{ Koord.spiel}}{=} g_i \circ d_{\sigma^{-1}(i)} \circ \phi = c_i \circ \phi^{-1} \circ \phi = c_i \circ \id = c_i \]
	und damit im Besonderen
		\[g_i \circ P(x) - g_i \circ P(x \mid \hat{x}_i) = g_i \circ d_{\hat{\jmath}} \circ \phi(x) - g_i \circ d_{\hat{\jmath}} \circ \phi(x \mid \hat{x}_i) = c_i(x) - c_i(x \mid \hat{x}_i) .\]
...
\begin{proof}
	...
	Weiter sind $(\id, \id): \Delta \to \Gamma$ und die Umkehrabbildung $(\id, \id): \Gamma \to \Delta$ monotone Morphismen, denn
	\begin{center}
		\begin{tikzcd}
		...
		\end{tikzcd}
	\end{center}		
	kommutieren offenkundig. Also sind $\Delta$ und $\Gamma$ tatsächlich monoton isomorph.
\end{proof}
...
\begin{defn}\label{def:PotentialMorphismen}
	...
	\begin{itemize}
		...
		\[\phi_j(\arg \min_{\hat{x}_{\sigma(j)} \in X_{\sigma(j)}}c_{\sigma(j)}(x \mid \hat{x}_{\sigma(j)})) \subseteq \arg \min_{y_j \in Y_j} d_j(\phi(x) \mid y_j)\]
	\end{itemize}	
...
\begin{bsp}\label{bsp:GegenbspSkalUndIsoIstSkalIso}
	Für skalierende Morphismen betrachten wir die folgenden beiden \mbox{2-}\nobreak Per\-so\-nen\-spie\-le:
	\begin{center}
		$\Gamma:$ \quad
		\begin{tabular}{c||c|c}
			& l 		& r 		\\\hline\hline
...
Während, wie man leicht zeigen kann, die Verknüpfung zweier kostenerhaltender Morphismen wieder einen kostenerhaltenden Morphismus ergibt und die zweier monotoner wieder einen monotonen, ist dies bei exakten/gewichtenden/\dots{} Morphismen im Allgemeinen nicht der Fall: 
...
Insbesondere erhält man also keine Kategorie von Spielen im Sinne der Kategorientheorie, wenn man sich nur auf exakte/gewichtende/\dots{} Morphismen beschränkt (was für kostenerhaltende/monotone Morphismen sehr wohl der Fall ist). Man kann allerdings sicherstellen, dass die Komposition zweier exakter/gewichtender/ordinaler/Beste-Antwort-Morphismen wieder die jeweilige Eigenschaft hat, wenn man sich auf Morphismen mit injektiver Spielerabbildung beschränkt:
...
\begin{bsp}\label{bsp:GegenbspKompositionSkMorphismen}
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
	Dass $(\id, \id): \Gamma \to \Delta$ ein skalierender Morphismus ist, haben wir bereits in \Cref{bsp:GegenbspSkalUndIsoIstSkalIso} gesehen und $(\id, \id): \Delta \to \Epsilon$ ist ein skalierender Morphismus mit Skalierungsfunktionen $f'_1 \coloneqq \id$ und $f'_2 \coloneqq \id$ (also sogar ein exakter Morphismus). 
...
\begin{proof}
	Besitzt ein Spiel $\Gamma= (I, X, (c_i))$ ein exaktes/gewichtendes/ordinales/Beste-Antwort-Potential $P$, so definieren wir das Koordinationsspiel $\Kappa = (I, X, (P))$. Dieses ist offensichtlich isomorph zu $\Gamma$ mittels des Identitätsmorphismus, wobei sich die entsprechende Eigenschaft des Morphismus aus der des Potentials ergibt.
	
	Ist umgekehrt $(\sigma, \phi): \Gamma \to \Kappa$ ein exakter/gewichtender/ordinaler/Beste-Antwort-I\-so\-mor\-phis\-mus von $\Gamma$ in ein Koordinationsspiel $\Kappa$ mit einheitlicher Kostenfunktion $p$, dann ist $P \coloneqq p \circ \phi$ ein entsprechendes Potential auf $\Gamma$.
	
	Völlig analog beweist man die Aussage für skalierende bzw. verallgemeinerte ordinale Potentiale.
\end{proof}
...
\begin{proof}
	Nach \Cref{prop:CharPotentialeDurchIsos} ist $\Delta$ exakt/gewichtend/\dots{} isomorph zu einem Koordintionsspiel und da wir laut \Cref{lemma:KompositionVonMorphismen} diese Isomorphismen verknüpfen können, gilt dies auch für $\Gamma$, welches damit -- erneut nach \Cref{prop:CharPotentialeDurchIsos} -- ein exaktes/gewichtetes/\dots{} Potential besitzt.
\end{proof}
...
\begin{prop}
	...
	\begin{center}
		...
		\node[right of=ex, node distance=7em] (gew) {gewichtend};
		\node[right of=gew, node distance=7em] (sk) {skalierend};
	...
	\begin{center}
		...
		\node[right of=ex, node distance=5em] (gew) {gewichtend};
		\node[right of=gew, node distance=6em] (sk) {skalierend};
...
\begin{proof}
	...
	\begin{description}
		\item[monoton $\bm{\Rightarrow}$ skalierend:] mit dem gleichen Beweis wie \Cref{prop:MonIsoKoordDannskPot} (wir haben dabei nirgends verwendet, dass eines der beteiligten Spiele ein Koordinationsspiel ist)
...
Ordinale Morphismen mit surjektiver Spielerabbildung erhalten also Verbesserungspfade und reflektieren Nichtverschlechterungspfade. Daraus folgt dann insbesondere \Cref{prop:ordPotVerbpfad}, der dies für (verallgemeinerte) ordinale Potentiale behauptet.
...
Zur Notwendigkeit der Bijektivität der Spielerabbildung betrachten wir das folgende Beispiel:
...
\begin{bsp}
	...
	\begin{itemize}
		\item Der Morphismus $(\sigma, \phi): \Gamma \to \Delta$ mit $\sigma(1) = 1$ und $\phi_1(t) = a, \phi_1(b) = b$ ist ein ordinaler Morphismus, dessen Spielerabbildung nicht surjektiv ist. Und tatsächlich gilt hier die Aussage von \Cref{kor:ReflektNG} nicht: Denn $(b,l)$ ist kein Nash-Gleichgewicht von $\Gamma$, obwohl $\phi(b,l) = b$ ein Nash-Gleichgewicht von $\Delta$ ist.
...
Aus \Cref{kor:ReflektNG} folgt insbesondere, dass Nash-Gleichgewichte in dem von einem verallgemeinerten ordinalen Potential induzierten Koordinationsspiel auch Nash-Gleichgewichte im Ausgangsspiel sind. Also sind verallgemeinerte ordinale Potentiale verallgemeinerte Nash-Potentiale.
...
Beste-Antwort-isomorphe Spiele haben damit sogar die gleichen Nash-Gleichgewichte, womit Beste-Antwort-Potentiale lokale Nash-Potentiale sind und wir den Beweis von \Cref{satz:ZusammenaengePotentiale} vervollständigt haben. Außerdem folgt hieraus \Cref{prop:BAPotBAPfad}.
...
Für monotone Morphismen gibt es einen zu \Cref{prop:NVReflVerbErh} analogen Satz über Nichtverschlechterungs- und Verbesserungspfade.
...
\begin{proof}
	Sei zunächst $x \in X$ ein Nash-Gleichgewicht in $\Gamma$ und $y_j \in Y_j$. Da $\phi_j$ surjektiv ist, gibt es dann ein $\hat{x}_{\sigma(j)}$ mit $\phi_j(\hat{x}_{\sigma(j)}) = y_j$. Wegen der Injektivität von $\sigma$ gilt ferner $\phi(x \mid \hat{x}_{\sigma(j)}) = (\phi(x) \mid \phi_j(\hat{x}_{\sigma(j)})) = \left(\phi(x) \mid y_j\right)$.
	
	Da $x$ ein Nash-Gleichgewicht ist, gilt nun $c_{\sigma(j)}(x) \leq c_{\sigma(j)}(x \mid \hat{x}_{\sigma(j)})$ und folglich wegen der Biordinalität von $\phi$ auch $d_j(\phi(x)) \leq d_j(\phi(x \mid \hat{x}_{\sigma(j)})) = d_j(\phi(x) \mid y_j)$. Also ist $\phi(x)$ ein Nash-Gleichgewicht in $\Delta$.
...
Mit diesem Satz können wir nun die in \Cref{beob:endlicheSpiele} gemachte Feststellung formalisieren (und beweisen), dass Spieler mit nur einer einzigen Strategie beim Untersuchen von Nash-Gleichgewichten immer ignoriert werden können:
...
\begin{proof}
	...
	\end{align*}	
	Dieser ist offensichtlich kostenerhaltend, also insbesondere biordinal, die Spielerabbildung ist injektiv, die Strategieabbildungen sind surjektiv und $I\setminus \sigma(I') = I \setminus I'$ enthält nur Dummy-Spieler. Damit folgt die Behauptung mit \Cref{prop:DummySpielerWeglassen}.
\end{proof}

Schließlich wollen wir noch das Analog zu \Cref{beob:ZshExGewPot} für Morphismen beweisen:

\begin{lemma}
	Seien $\Gamma = (I, X, (c_i))$ und $\Delta = (J, Y, (d_j))$ zwei Spiele. Dann gibt es genau dann einen gewichtenden Morphismus $\Gamma \to \Delta$, wenn es positive Gewichte $(w_j)_{j\in J}$ und einen exakten Morphismus $\Gamma \to \Delta_w \coloneqq (J, Y, (w_j\cdot d_j))$ gibt.
\end{lemma}
...
\begin{proof}
	...
	Und dies ist offensichtlich äquivalent zu
		\[c_{\sigma(j)}(x) - c_{\sigma(j)}(x \mid \hat{x}_{\sigma(j)}) = w_j \cdot \left(d_j(\phi(x)) - d_j(\phi(x \mid \hat{x}_{\sigma(j)}))\right),\]
