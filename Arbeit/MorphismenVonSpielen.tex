...
\begin{defn}
	Ein Spiel heißt \emph{Koordinationsspiel}\todo{\cite{KoordDummy}}, wenn alle Spieler eine gemeinsame Kostenfunktion verwenden.
\end{defn}
...
\todo[inline]{Dass das funktioniert liegt daran, dass die Spielerabbildung in die umgekehrte Richtung geht!}

\todo[inline]{Später werden wir auch Abbildungen $\phi_i: X_i \to Y_i$ zwischen den spielerspezifischen Strategieräumen betrachten und die durch diese induzierte Abbildung $X \to Y: x = (x_i)_{i \in I} \mapsto \phi(x) := (\phi_i(x_i))_{i \in I}$ mit $\phi$ bezeichnen. Analog zur Notation für Strategieräume werden wir außerdem die Notation $\phi_{-i}: X_{-i} \to Y_{-i}$ verwenden.}
