...
Darüber hinaus helfen Morphismen dabei auch, andere Beziehungen zwischen verschiedenen Spielen zu verstehen. Bereits \citeauthor{Nash} verwendet in \cite[S. 288f]{Nash} den Begriff eines Automorphismus (bzw. einer Symmetrie) von Spielen. Auch die von \citeauthor{MonShap} in \cite[Theorem 3.2]{MonShap} gefundene Äquivalenz von Auslastungsspielen und Spielen mit exaktem Potential wird durch einen passenden Isomorphiebegriff beschrieben. In \Cref{sec:Auslastungsspiele} werden wir später noch weitere Beispiele für derartige Verbindungen kennenlernen. Schließlich ist es aus kategorientheoretischer Sicht auch allgemein interessant, die Morphismen der Kategorie, in der man arbeitet, zu verstehen, da diese die Struktur dieser Kategorie festlegen und etwa dazu verwendet werden, abstrakte Konstruktionen durchzuführen. Diesen Ansatz werden wir hier allerdings nicht fortführen und verweisen dazu auf \cite{LapGameCat}.
...
\begin{bsp}\label{bsp:GegenbspKompositionSkMorphismen}
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
	Dass $(\id, \id): \Gamma \to \Delta$ ein skalierender Morphismus ist, haben wir bereits in \Cref{bsp:GegenbspSkalUndIsoIstSkalIso} gesehen und $(\id, \id): \Delta \to \Epsilon$ ist ein skalierender Morphismus mit Skalierungsfunktionen $f'_1 \coloneqq \id$ und $f'_2 \coloneqq \id$. 
