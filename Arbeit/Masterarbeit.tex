...
Nachdem wir in \Cref{sec:Grundlagen} einige Grundbegriffe der Spieltheorie definiert haben, werden wir uns zunächst in \Cref{sec:Potentiale} dem Konzept der Potentialfunktionen für Spiele zuwenden. Wir werden eine ganze Reihe derartiger Potentiale kennenlernen und jeweils notwendige und hinreichende Bedingungen dafür zeigen, dass ein Spiel ein entsprechendes Potential besitzt. Im folgenden \Cref{sec:Morphismen} werden wir verschiedene Arten von Morphismen zwischen Spielen definieren, die dann Isomorphien induzieren, die wir nutzen können um die besagten Beziehungen zwischen verschiedenen Spielen beschreiben zu können. In \Cref{sec:Auslastungsspiele} schließlich werden wir diese (Iso-)Morphismen nutzen um einige Zusammenhänge zwischen unterschiedlichen Klassen von Auslastungs- und Potentialspielen zu formulieren und zu beweisen.
...
Offene Fragen: 
\begin{itemize}
	\item Zu ordinalen/Beste-Antwort-Potentialen passende Variante von Auslastungsspielen? Vermutlich schwerer, da diese Potentiale das Spiel nicht mehr bis auf exakte Isomorphie/bis auf ein Dummy-Spiel festlegen - es kann also nicht das Potential alleine in der Konstruktion verwendet werden, sondern es muss auch das ursprüngliche Spiel miteingehen. Das erfordert vermutlich eine neue Konstruktionsidee.
	\item Gew. Auslastungsspiele mit affin exponentiellen Ressourcenkosten direkt in kostengewichtete Auslastungsspiele umwandeln (analog zu \Cref{satz:GewAuslZuUngewAusl})
	\item Weitere schöne Konstruktionen? Evtl. Gegenbeispiel erwähnen
	\item Offene Fragen aus Kapitel 4.1.4 aufgreifen.
\end{itemize}
