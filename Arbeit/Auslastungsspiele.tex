...
Nachdem wir die Äquivalenz von ungewichteten Auslastungs- und exakten Potentialspielen in \Cref{satz:MondererShapleyKostengew} bereits auf kostengewichtete Auslastungs- und gewichtete Potentialspiele übertragen haben, werden wir nun untersuchen inwieweit sich dies auch für skalierte Auslastungs- und Potentialspiele fortsetzen lässt. Die eine Richtung gilt dabei weiterhin mit nur einem leicht schwächeren Isomorphiebegriff und einer Zusatzbedingung an die verwendeten Skalierungsfunktionen:

\begin{satz}
	Jedes skalierte $N$-Personen-Potentialspiel, dessen Skalierungsfunktionen $0$ auf $0$ abbilden\todo{Kann man das oBdA bis auf exakte Isomorphie annehmen?}, ist exakt isomorph zu einem skalierten Auslastungsspiel.
\end{satz}
...
\begin{proof}
	...
	&=\sum_{r \in R_K: x_{\hat{\imath}} \in r_{\hat{\imath}}} f_{\hat{\imath}} \circ g_r(\underbrace{l_r(\phi(x))}_{\neq 0 \iff r = \left(\{x_i\}\right)_{i \in I}}) -
	\sum_{r \in R_K: \hat{x}_{\hat{\imath}} \in r_{\hat{\imath}}} f_{\hat{\imath}} \circ g_r(\underbrace{l_r(\phi(x \mid \hat{x}_i))}_{\neq 0 \iff r = \left(\{x_i\}\right)\mid\{\hat{x}_i\}}) = \\
